...
.env.example → .env로 복사한 뒤 값만 채우면 됩니다.
...
11) API 서버 에러 응답 규격
API 서버는 모든 오류 상황에 대해 일관된 JSON 형식의 응답을 반환합니다.
...
404	RESOURCE_NOT_FOUND	리소스가 존재하지 않음
404	USER_NOT_FOUND	사용자 존재하지 않음
409	DUPLICATE_RESOURCE	중복 데이터(이메일 중복 등)
409	STATE_CONFLICT	리소스 상태 충돌
422	UNPROCESSABLE_ENTITY	논리적 오류로 처리 불가
...
11-3. 에러 처리 규칙
모든 예외는 위 표의 코드 중 하나로 매핑 (필요 시 프로젝트 사양에 맞춰 추가 가능)

Swagger 문서의 각 엔드포인트에 최소 400/401/403/404/422/500 예시 포함(권장)

Postman 테스트에서 대표 에러 케이스 검증 요청 3개 이상 포함(권장)

Validation 실패 시 details에 필드별 오류 포함
